...
//' . __("Filter the BOT :") . ' <input type="checkbox" name="bot" /> - 
 ...
 echo __("Filter by error :") . select("microsite_main", "answer", $data['answer'], $data['id_answer'], "textform lg translation") . ' - ';
 echo __("Activate")." / ".__("Deactivate")." " . select("microsite_main", "answer", $data['answer'], $data['id_answer'], "textform lg translation") . ' ';

 
 echo "<input class=\"button btBlueTest overlayW btMedium\" type=\"submit\" value=\"" . __("Filter") . "\" /> - ";

 echo '<a class="button btBlueTest overlayW btMedium" href="'.LINK.'microsite/export/">'.__("Export CSV").'</a>';
